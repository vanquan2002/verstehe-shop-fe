...
const Contents = ({ id, product }) => {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (checkLineElementDescriptionRef.current) {
        const elementHeight =
          checkLineElementDescriptionRef.current.scrollHeight;
  ...
  }, [checkLineElementDescriptionRef]);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-4">
      ...
      <div className="col-span-1">
              ...
              {product.sizes?.map((item, i) => (
                ...
                >
                  <button
                    ...
                    className={`uppercase ${
                      item === size ? "text-darkPrimary" : "text-whitePrimary"
                    }`}
                  >
                    {item}
                  </button>
          ...
          <div className="w-full col-span-1">
            <div
              className={`flex items-center ${
                product.countInStock === 0 && "opacity-30 pointer-events-none"
              }`}
            >
              <button
                className={`${
        ...
        <div className="flex items-start mb-5">
          ...
          <div className="w-full">
            <p className="text-white uppercase font-medium">Pre-oder 7 ngày</p>
          </div>
        </div>
        <div className="flex items-start mb-5">
          <div className="w-full">
          ...
          <div className="w-full">
            <p className="text-white uppercase font-medium">{product.color}</p>
          </div>
        </div>
        <div className="flex items-start mb-5">
          <div className="w-full">
          ...
          <div className="w-full">{formatDataWithBr(product.description)}</div>
        </div>
        <div className="flex items-start">
          ...
          <div className="w-full flex items-center gap-3">
            ...
            <p className="text-white font-medium">( {product.numReviews} )</p>
          </div>
        </div>
      </div>
    </div>
